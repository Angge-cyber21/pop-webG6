...
// Store selected platter in session
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['platter'])) {
    $_SESSION['selected_platter'] = $_POST['platter'];
}

// Get the selected platter from session
...
<body>
    ...
    <section class="platter-section">
        ...
        <button type="button" class="continue-button" onclick="window.location.href='foods_drinks.php'">Go Back</button>
    </section>
    ...
    <script>
        ...
        continueButton.addEventListener('click', function (event) {
            ...
            Swal.fire({
                ...
                text: 'Would you like to pay online or at the counter?',
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    ...
                    // Show order summary in SweetAlert
                    const platter = <?= json_encode($platterContents); ?>[<?php echo json_encode($selectedPlatter); ?>];
                    const orderSummary = platter.items.join('<br>');
                    Swal.fire({
                        title: 'Order Summary',
                        html: `
                            <h3>${platter.title}</h3>
                        ...
                        if (confirmResult.isConfirmed) {
                            // Send order summary to the server (AJAX)
                            const orderDetails = {
                            ...
                            .then(data => {
                                Swal.fire('Order Confirmed!', 'The order has been sent to your email.', 'success');
                            })
                            .catch(error => {
                                console.error('Error sending email:', error);
                                Swal.fire('Error!', 'There was a problem sending your order summary via email.', 'error');
                            });
                        }
                    });
                }
